...
from posts.models.post import Post, Repost
...
def profile_view(request, username):
    ...
    can_view_details = (current_user == profile_user or not profile_user.is_private or is_following) and not blocked_between

    posts = profile_user.posts.all() if can_view_details else []
    posts_count = posts.count() if can_view_details else 0
...
# --- нижче залишив усі фічі з другої гілки ---
@login_required
@require_POST
...
def follow_request_accept_view(request, pk):
    ...
    entries = list(
        FollowRequest.objects.select_related("from_user").filter(to_user=request.user, status=FollowRequest.PENDING)
    )
...
def followers_list_view(request, username):
    ...
    return render(request, "users/_followers_list.html", {"profile_user": profile_user, "users": users, "is_me": is_me})
...
def following_list_view(request, username):
    ...
    is_me = request.user == profile_user
    return render(request, "users/_following_list.html", {"profile_user": profile_user, "users": users, "is_me": is_me})
...
def follow_requests_incoming_view(request, username):
    if request.user.username != username:
        raise Http404()
    q = FollowRequest.objects.select_related("from_user").filter(to_user=request.user, status=FollowRequest.PENDING)
...
def follow_requests_outgoing_view(request, username):
    if request.user.username != username:
        raise Http404()
    q = FollowRequest.objects.select_related("to_user").filter(from_user=request.user, status=FollowRequest.PENDING)
    entries = [fr for fr in q]
...
def follow_request_cancel_view(request, username):
    ...
    FollowRequest.objects.filter(from_user=request.user, to_user=to_user, status=FollowRequest.PENDING).delete()
    ...
    if request.headers.get("HX-Request") == "true" or request.headers.get("X-Requested-With") == "XMLHttpRequest":
        ...
        entries = list(
            FollowRequest.objects.select_related("to_user").filter(from_user=request.user, status=FollowRequest.PENDING)
        )
